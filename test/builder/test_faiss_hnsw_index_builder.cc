...
TEST_F(FaissHnswIndexBuilderTest, Add) {
  ...
  std::make_unique<FaissHnswIndexBuilder>(faiss_hnsw_meta())->EnableCustomRowId().Open().
  ...
  std::make_unique<FaissHnswIndexBuilder>(faiss_hnsw_meta())->Open().
  ...
  EXPECT_THROW(const_cast<uint32_t*>(base_vl_view().offsets)[1] = 0;
               std::make_unique<FaissHnswIndexBuilder>(faiss_hnsw_meta())->Open().Add({base_vl_view()}), Error);
