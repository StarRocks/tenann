...
//   std::shared_ptr<Index> index_ref = index_builder1->index_ref();
//   EXPECT_TRUE(index_ref != nullptr);
//   EXPECT_EQ(index_ref->index_type(), IndexType::kFaissHnsw);

//   auto index_builder2 = std::make_unique<FaissHnswIndexBuilder>(meta_);
//   EXPECT_THROW(index_builder2->Open().Add({base_view()}, row_ids().data()), tenann::Error);
// }
...
//     EXPECT_THROW(index_builder()->BuildWithPrimaryKey(input_columns, 0), Error);
//   }

//   // InvalidDataColType
//   EXPECT_THROW(
//       // set SeqViewType::kPrimitiveSeqView to input_columns[data_col_index].seq_view_type
//       index_builder()->BuildWithPrimaryKey({id_view(), id_view()}, 0), Error);

//   // InvalidDataColElemType
//   EXPECT_THROW(auto new_base_view = base_view();
//                new_base_view.elem_type = PrimitiveType::kDoubleType;
//                index_builder()->BuildWithPrimaryKey({id_view(), new_base_view}, 0), Error);

//   // ContainsDimFalse
//   EXPECT_THROW(auto new_meta = meta(); new_meta.common_params().erase("dim");
//                std::unique_ptr<IndexBuilder> faiss_hnsw_index_builder =
//                    std::make_unique<FaissHnswIndexBuilder>(new_meta);
//                faiss_hnsw_index_builder->BuildWithPrimaryKey({id_view(), base_view()}, 0), Error);
...
//   // InvalidMetricType
//   EXPECT_THROW(auto new_meta = meta();
...
//   // InvalidM, M == -1
//   EXPECT_THROW(auto new_meta = meta(); new_meta.index_params()["M"] = -1;
...
//   EXPECT_THROW(auto new_base_vl_view = base_vl_view(); new_base_vl_view.offsets[1] = 0;
//                index_builder()->BuildWithPrimaryKey({id_view(), new_base_vl_view}, 0), Error);
// }

// TEST_F(FaissHnswIndexBuilderTest, BuildImpl) {
//   auto index_builder = std::make_unique<FaissHnswIndexBuilder>(meta_);
//   index_builder->Open().Add({base_view()});
...
//   {
//     std::vector<SeqView> input_columns;
//     EXPECT_THROW(index_builder()->Build(input_columns), Error);
//   }

//   // InvalidDataColType
//   EXPECT_THROW(
//       // set SeqViewType::kPrimitiveSeqView to input_columns[0].seq_view_type
//       index_builder()->Build({id_view()}), Error);

//   // InvalidDataColElemType
//   EXPECT_THROW(auto new_base_view = base_view();
//                new_base_view.elem_type = PrimitiveType::kDoubleType;
//                index_builder()->Build({new_base_view}), Error);

//   // ContainsDimFalse
//   EXPECT_THROW(auto new_meta = meta(); new_meta.common_params().erase("dim");
//                std::unique_ptr<IndexBuilder> faiss_hnsw_index_builder =
//                    std::make_unique<FaissHnswIndexBuilder>(new_meta);
//                faiss_hnsw_index_builder->Build({base_view()}), Error);
...
//   // DataColVlArraySeqViewSliceSizeError
//   EXPECT_THROW(auto new_base_vl_view = base_vl_view(); new_base_vl_view.offsets[1] = 0;
//                index_builder()->Build({new_base_vl_view}), Error);
// }
...
//   EXPECT_THROW(
//       auto new_id_view = id_view(); new_id_view.elem_type = PrimitiveType::kFloatType;
