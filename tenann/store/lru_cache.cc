...
uint32_t ShardedLRUCache::_shard(uint32_t hash) { return hash >> (32 - kNumShardBits); }
...
void* ShardedLRUCache::value(Handle* handle) { return reinterpret_cast<LRUHandle*>(handle)->value; }
...
size_t ShardedLRUCache::get_capacity() { return _get_stat(&LRUCache::get_capacity); }
...
size_t ShardedLRUCache::get_lookup_count() { return _get_stat(&LRUCache::get_lookup_count); }
...
void ShardedLRUCache::get_cache_status(json* document) {
  ...
  for (uint32_t i = 0; i < shard_count; ++i) {
    ...
    shard_info["usage_ratio"] = usage_ratio;

    size_t lookup_count = _shards[i].get_lookup_count();
    size_t hit_count = _shards[i].get_hit_count();
    shard_info["lookup_count"] = lookup_count;
    shard_info["hit_count"] = hit_count;
    ...
    shard_info["hit_ratio"] = hit_ratio;

    document->push_back(shard_info);
  }
}
