...
namespace dummy_faiss {

/*****************************************
 * IndexIVFPQ implementation
 ******************************************/

IndexIVFPQ::IndexIVFPQ(Index* quantizer, size_t d, size_t nlist, size_t M, size_t nbits_per_idx,
                       MetricType metric)
...
void IndexIVFPQ::train_residual(idx_t n, const float* x) { train_residual_o(n, x, nullptr); }

void IndexIVFPQ::train_residual_o(idx_t n, const float* x, float* residuals_2) {
  const float* x_in = x;

  x = fvecs_maybe_subsample(d, (size_t*)&n, pq.cp.max_points_per_centroid * pq.ksub, x, verbose,
                            pq.cp.seed);
  ...
  if (do_polysemous_training) {
    if (verbose) printf("doing polysemous training for PQ\n");
  ...
  if (residuals_2) {
    ...
    for (idx_t i = 0; i < n; i++) {
      ...
      for (int j = 0; j < d; j++) res[j] = xx[j] - res[j];
    }
  }
...
void IndexIVFPQ::encode_multiple(size_t n, idx_t* keys, const float* x, uint8_t* xcodes,
                                 bool compute_keys) const {
  if (compute_keys) quantizer->assign(n, x, keys);

  encode_vectors(n, x, keys, xcodes);
}

void IndexIVFPQ::decode_multiple(size_t n, const idx_t* keys, const uint8_t* xcodes,
                                 float* x) const {
...
void IndexIVFPQ::encode_vectors(idx_t n, const float* x, const idx_t* list_nos, uint8_t* codes,
...
void IndexIVFPQ::add_core_o(idx_t n, const float* x, const idx_t* xids, float* residuals_2,
...
void IndexIVFPQ::reconstruct_from_offset(int64_t list_no, int64_t offset, float* recons) const {
...
void initialize_IVFPQ_precomputed_table(int& use_precomputed_table, const Index* quantizer,
                                        ...
                                        AlignedTable<float>& precomputed_table, bool verbose) {
  ...
  if (use_precomputed_table == 0) {  // then choose the type of table
    ...
    const MultiIndexQuantizer* miq = dynamic_cast<const MultiIndexQuantizer*>(quantizer);
  ...
  for (int m = 0; m < pq.M; m++)
    for (int j = 0; j < pq.ksub; j++)
      r_norms[m * pq.ksub + j] = fvec_norm_L2sqr(pq.get_centroids(m, j), pq.dsub);
  ...
  if (use_precomputed_table == 1) {
    ...
    for (int m = 0; m < cpq.M; m++) {
      for (size_t i = 0; i < cpq.ksub; i++) {
        memcpy(centroids.data() + i * d + m * cpq.dsub, cpq.get_centroids(m, i),
               sizeof(*centroids.data()) * cpq.dsub);
      }
    }

    pq.compute_inner_prod_tables(cpq.ksub, centroids.data(), precomputed_table.data());
...
void IndexIVFPQ::precompute_table() {
  initialize_IVFPQ_precomputed_table(use_precomputed_table, quantizer, pq, precomputed_table,
                                     verbose);
}
...
InvertedListScanner* IndexIVFPQ::get_InvertedListScanner(bool store_pairs,
...
void IndexIVFPQStats::reset() { memset(this, 0, sizeof(*this)); }
...
struct CodeCmp {
  const uint8_t* tab;
  size_t code_size;
  bool operator()(int a, int b) const { return cmp(a, b) > 0; }
