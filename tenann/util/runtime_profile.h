...
//   https://github.com/StarRocks/starrocks/blob/main/be/src/util/runtime_profile.h
...
#include "tenann/common/compiler_util.h"
...
class RuntimeProfile {
 public:
  class Counter {
   public:
    ...
    static TCounterStrategy create_strategy(
        ...
        int64_t display_threshold = 0) {
      auto aggregate_type =
          is_time_type(type) ? TCounterAggregateType::AVG : TCounterAggregateType::SUM;
    ...
    void bit_or(int64_t delta) {
      ...
      do {
        ...
        if (LIKELY((old | delta) == old)) return;  // Bits already set, avoid atomic.
          ...
          UNLIKELY(!_value.compare_exchange_strong(old, old | delta, std::memory_order_relaxed)));
    }

    virtual void set(int64_t value) { _value.store(value, std::memory_order_relaxed); }

    virtual void set(double value) {
      _value.store(static_cast<int64_t>(value), std::memory_order_relaxed);
    }

    virtual int64_t value() const { return _value.load(std::memory_order_relaxed); }

    virtual double double_value() const {
      return static_cast<double>(_value.load(std::memory_order_relaxed));
    }
  ...
  class HighWaterMarkCounter : public Counter {
   public:
    ...
    explicit HighWaterMarkCounter(TUnit::type type, const TCounterStrategy& strategy,
                                  int64_t value = 0)
    ...
    bool try_add(int64_t delta, int64_t max) {
      while (true) {
        ...
        if (LIKELY(current_value_.compare_exchange_strong(old_val, new_val,
   ...
   private:
    ...
    void UpdateMax(int64_t v) {
      while (true) {
        ...
        if (LIKELY(_value.compare_exchange_strong(old_max, new_max, std::memory_order_relaxed)))
          break;
      }
    }
  ...
  Counter* add_child_counter(const std::string& name, TUnit::type type,
  ...
  void copy_all_counters_from(RuntimeProfile* src_profile,
                              const std::string& attached_counter_name = ROOT_COUNTER);
 ...
 private:
  ...
  Counter* add_counter_unlock(const std::string& name, TUnit::type type,
                              const TCounterStrategy& strategy, const std::string& parent_name);
 ...
 public:
  ...
  static RuntimeProfile* merge_isomorphic_profiles(ObjectPool* obj_pool,
 ...
 private:
  ...
  static bool is_time_type(TUnit::type type) {
    return  // TUnit::type::CPU_TICKS == type ||
        TUnit::type::TIME_NS == type || TUnit::type::TIME_MS == type || TUnit::type::TIME_S == type;
  }
...
class ScopedTimer {
 public:
  explicit ScopedTimer(RuntimeProfile::Counter* counter, const bool* is_cancelled = nullptr)
    ...
    T_DCHECK(counter->type() == TUnit::TIME_NS);
    _sw.start();
  }
