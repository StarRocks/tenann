...
//   https://github.com/StarRocks/starrocks/blob/main/be/src/util/pretty_printer.h
...
#include "tenann/util/template_util.h"
...
class PrettyPrinter {
 public:
  ...
  static ENABLE_IF_ARITHMETIC(T, std::string)
    ...
    switch (unit) {
      ...
      default:
        T_DCHECK(false) << "Unsupported TUnit: " << value;
        break;
    }
    return ss.str();
  }
 ...
 private:
  ...
  static void print_timems(T value, std::stringstream* ss) {
    T_DCHECK_GE(value, static_cast<T>(0));
    if (value == 0) {
      *ss << "0";
    } else {
      bool hour = false;
