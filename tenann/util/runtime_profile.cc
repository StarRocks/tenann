...
//   https://github.com/StarRocks/starrocks/blob/main/be/src/util/runtime_profile.cpp
...
void RuntimeProfile::merge(RuntimeProfile* other) {
  ...
  // Merge this level
  {
    ...
    for (src_iter = other->_counter_map.begin(); src_iter != other->_counter_map.end();
      ...
      if (dst_iter == _counter_map.end()) {
        _counter_map[src_iter->first] =
            std::make_pair(_pool->add(new Counter(src_iter->second.first->type(),
                                                  src_iter->second.first->strategy(),
                                                  src_iter->second.first->value())),
                           src_iter->second.second);
      } else {
        T_DCHECK(dst_iter->second.first->type() == src_iter->second.first->type());

        if (dst_iter->second.first->type() == TUnit::DOUBLE_VALUE) {
          double new_val =
              dst_iter->second.first->double_value() + src_iter->second.first->double_value();
    ...
    for (child_counter_src_itr = other->_child_counter_map.begin();
      ...
      auto& child_counters = LookupOrInsert(&_child_counter_map, child_counter_src_itr->first,
                                            std::set<std::string>());
...
void RuntimeProfile::add_child_unlock(RuntimeProfile* child, bool indent,
                                      ChildVector::iterator pos) {
  T_DCHECK(child != nullptr);
  T_DCHECK(child->_parent == nullptr);
  T_DCHECK(_child_map.find(child->_name) == _child_map.end());
  _child_map[child->_name] = child;
  _children.insert(pos, std::make_pair(child, indent));
  child->_parent = this;
}

RuntimeProfile::Counter* RuntimeProfile::add_counter_unlock(const std::string& name,
                                                            TUnit::type type,
  ...
  T_DCHECK(parent_name == ROOT_COUNTER || _counter_map.find(parent_name) != _counter_map.end());
...
void RuntimeProfile::copy_all_info_strings_from(RuntimeProfile* src_profile) {
  ...
  for (const auto& [key, value] : src_profile->_info_strings) {
    ...
    if (exist_ptr == nullptr) {
      ...
      size_t i = 0;
      while (true) {
        const std::string indexed_key = fmt::format("{}__DUP({})", original_key, i++);
...
#define ADD_COUNTER_IMPL(NAME, T)                                                                  \
  RuntimeProfile::T* RuntimeProfile::NAME(const std::string& name, TUnit::type unit,               \
                                          const TCounterStrategy& strategy,                        \
                                          const std::string& parent_name) {                        \
    T_DCHECK_EQ(_is_averaged_profile, false);                                                      \
    std::lock_guard<std::mutex> l(_counter_lock);                                                  \
    ...
    _counter_map[name] = std::make_pair(counter, parent_name);                                     \
    auto& child_counters =                                                                         \
        LookupOrInsert(&_child_counter_map, parent_name, std::set<std::string>());                 \
...
RuntimeProfile::Counter* RuntimeProfile::add_child_counter(const std::string& name,
                                                           TUnit::type type,
                                                           const TCounterStrategy& strategy,
                                                           const std::string& parent_name) {
  std::lock_guard<std::mutex> l(_counter_lock);
...
RuntimeProfile::ThreadCounters* RuntimeProfile::add_thread_counters(const std::string& prefix) {
  ...
  counter->_total_time = add_counter(prefix + THREAD_TOTAL_TIME, TUnit::TIME_NS,
                                     Counter::create_strategy(TUnit::TIME_NS));
  counter->_user_time =
      add_child_counter(prefix + THREAD_USER_TIME, TUnit::TIME_NS,
                        Counter::create_strategy(TUnit::TIME_NS), prefix + THREAD_TOTAL_TIME);
  counter->_sys_time =
      add_child_counter(prefix + THREAD_SYS_TIME, TUnit::TIME_NS,
                        Counter::create_strategy(TUnit::TIME_NS), prefix + THREAD_TOTAL_TIME);
...
std::pair<RuntimeProfile::Counter*, std::string> RuntimeProfile::get_counter_pair(
...
void RuntimeProfile::copy_all_counters_from(RuntimeProfile* src_profile,
                                            const std::string& attached_counter_name) {
  T_DCHECK(src_profile != nullptr);
  if (this == src_profile) {
    return;
  }
  ...
  while (!name_queue.empty()) {
    ...
    if (name != ROOT_COUNTER) {
      ...
      auto* new_counter =
          add_counter_unlock(name, src_counter->type(), src_counter->strategy(), parent_name);
...
void RuntimeProfile::pretty_print(std::ostream* s, const std::string& prefix) const {
  ...
  T_DCHECK(total_time != counter_map.end());
  ...
  if (total_time->second.first->value() != 0) {
    stream << "(Active: "
           << PrettyPrinter::print(total_time->second.first->value(),
                                   total_time->second.first->type())
...
json RuntimeProfile::to_json() const {
...
int64_t RuntimeProfile::units_per_second(const RuntimeProfile::Counter* total_counter,
  ...
  T_DCHECK(total_counter->type() == TUnit::BYTES || total_counter->type() == TUnit::UNIT);
  T_DCHECK(timer->type() == TUnit::TIME_NS);
...
RuntimeProfile* RuntimeProfile::merge_isomorphic_profiles(ObjectPool* obj_pool,
  ...
  auto* merged_profile =
      obj_pool->add(new RuntimeProfile(profiles[0]->name(), profiles[0]->_is_averaged_profile));
  ...
  // Merge counters
  {
    ...
    for (auto* profile : profiles) {
      ...
      while (!name_queue.empty()) {
        ...
        for (const auto& name : current_names) {
          ...
          T_DCHECK(pair_it != profile->_counter_map.end());
          ...
          if (counter->type() != exist_type) {
            T_LOG(WARNING) << "find non-isomorphic counter, profile_name=" << merged_profile->name()
                           << ", counter_name=" << name
                           << ", exist_type=" << std::to_string(exist_type)
    ...
    for (const auto& tuple : level_ordered_counters) {
      ...
      if (name.rfind(MERGED_INFO_PREFIX_MIN, 0) == 0 ||
          name.rfind(MERGED_INFO_PREFIX_MAX, 0) == 0) {
        continue;
      }
      ...
      for (auto profile : profiles) {
        ...
        auto* min_counter = profile->get_counter(fmt::format("{}{}", MERGED_INFO_PREFIX_MIN, name));
        ...
        auto* max_counter = profile->get_counter(fmt::format("{}{}", MERGED_INFO_PREFIX_MAX, name));
      ...
      if (ROOT_COUNTER != parent_name && merged_profile->get_counter(parent_name) != nullptr) {
        ...
        if (ROOT_COUNTER != parent_name) {
          T_LOG(WARNING) << "missing parent counter, profile_name=" << merged_profile->name()
      ...
      if (skip_merge) {
        ...
        auto* min_counter =
            merged_profile->add_child_counter(fmt::format("{}{}", MERGED_INFO_PREFIX_MIN, name),
                                              type, merged_counter->strategy(), name);
        auto* max_counter =
            merged_profile->add_child_counter(fmt::format("{}{}", MERGED_INFO_PREFIX_MAX, name),
                                              type, merged_counter->strategy(), name);
    ...
    for (auto i = 0; i < profiles[0]->_children.size(); i++) {
      ...
      for (auto profile : profiles) {
        if (i >= profile->num_children()) {
          T_LOG(WARNING) << "find non-isomorphic children, profile_name=" << merged_profile->name()
    ...
    for (const std::string& child_counter : child_counters) {
      ...
      if (display_threshold == 0 || (display_threshold > 0 && value > display_threshold)) {
        stream << prefix << "   - " << iter->first << ": "
               << PrettyPrinter::print(iter->second.first->value(), iter->second.first->type())
               << std::endl;
