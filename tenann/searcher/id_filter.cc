...
RangeIdFilter::RangeIdFilter(idx_t min_id, idx_t max_id, bool assume_sorted) {
  adapter_ = std::make_unique<IDSelectorRangeAdapter>(min_id, max_id, assume_sorted);
}
...
ArrayIdFilter::ArrayIdFilter(const idx_t* ids, size_t num_ids) {
  ...
  adapter_ = std::make_unique<IDSelectorArrayAdapter>(id_array_.size(), id_array_.data());
}
...
BatchIdFilter::BatchIdFilter(const idx_t* ids, size_t num_ids) {
  adapter_ = std::make_unique<IDSelectorBatchAdapter>(num_ids, ids);
}
...
BitmapIdFilter::BitmapIdFilter(const uint8_t* bitmap, size_t bitmap_size) {
  adapter_ = std::make_unique<IDSelectorBitmapAdapter>(bitmap_size, bitmap);
}
