...
# check TENANN_HOME
if [[ -z ${TENANN_HOME} ]]; then
    echo "Error: TENANN_HOME is not set"
    exit 1
fi

# include custom environment variables
if [[ -f ${TENANN_HOME}/custom_env.sh ]]; then
    . ${TENANN_HOME}/custom_env.sh
fi

# set TENANN_THIRDPARTY
if [[ -z ${TENANN_THIRDPARTY} ]]; then
    export TENANN_THIRDPARTY=${TENANN_HOME}/thirdparty
fi

# set cachelib dir
if [[ -z ${CACHELIB_DIR} ]]; then
    export CACHELIB_DIR=${TENANN_THIRDPARTY}/installed/cachelib
fi
...
export CLANG_COMPATIBLE_FLAGS=`echo | ${TENANN_GCC_HOME}/bin/gcc -Wp,-v -xc++ - -fsyntax-only 2>&1 \
